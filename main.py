...
async def on_ready():
    ...
    # TODO: make recurring task to update problemse
    await update_problemset()
    print('Problemset updated')
    if (db.get_potd() is None):
        await select_potd()
    print(await check_solved('lethan3', 1629, 'B'))
    print(await check_solved('lethan3', 1841, 'D'))
...
potd_difficulties = [800, 1000, 800, 900, 1000, 1400, 1200]
...
async def update_solvers():
    ...
    print(users)
    for user in users:
        if await check_solved(user[2], problem.id, problem.index):
